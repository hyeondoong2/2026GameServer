...
bool SESSION::process_packet(unsigned char* p)
{
    ...
    case C2S_LOGIN: {
        ...
        for (int dy = -1; dy <= 1; ++dy)
        {
            ...
            {
                int ny = m_sector_y + dy;
                int nx = m_sector_x + dx;

                if (ny >= 0 && ny < SECTOR_ROWS && nx >= 0 && nx < SECTOR_COLS)
                {
                    std::lock_guard<std::mutex> lock(g_sectors[ny][nx].m_mutex);
                    for (int other_id : g_sectors[ny][nx].players)
                    {
                        if (other_id == m_id) continue;

                        std::shared_ptr<SESSION> pl = clients[other_id].load();
                        if (nullptr == pl || pl->m_state != CS_PLAYING) continue;
    ...
    case C2S_MOVE: {
        ...
        int old_sector_x = m_sector_x;
        int old_sector_y = m_sector_y;
        ...
        if (old_sector_x != m_sector_x || old_sector_y != m_sector_y)
        {
            {
                std::lock_guard<std::mutex> lock(g_sectors[old_sector_y][old_sector_x].m_mutex);
                g_sectors[old_sector_y][old_sector_x].players.erase(m_id);
            }
            {
                std::lock_guard<std::mutex> lock(g_sectors[m_sector_y][m_sector_x].m_mutex);
                g_sectors[m_sector_y][m_sector_x].players.insert(m_id);
...
void disconnect(int key)
{
    ...
    if (nullptr != cl)
    {
        ...
        {
            std::lock_guard<std::mutex> lock(g_sectors[cl->m_sector_y][cl->m_sector_x].m_mutex);
            g_sectors[cl->m_sector_y][cl->m_sector_x].players.erase(key);
        }
