...
bool HandleLoginPacket(const char* clientID, float& outX, float& outY)
{
    SQLHSTMT hStmt;
    SQLAllocHandle(SQL_HANDLE_STMT, hDbc, &hStmt);

    char query[256];
    sprintf_s(query, "SELECT PosX, PosY FROM PlayerInfo WHERE PlayerID = '%s'", clientID);
    ...
    {
        SQLGetData(hStmt, 1, SQL_C_FLOAT, &outX, 0, NULL);
        SQLGetData(hStmt, 2, SQL_C_FLOAT, &outY, 0, NULL);
...
void SavePlayerPosition(const char* clientID, float curX, float curY)
{
    if (clientID == nullptr || clientID[0] == '\0') return;
    ...
    {
        char query[256] = { 0 };
        sprintf_s(query, "UPDATE PlayerInfo SET PosX = %f, PosY = %f WHERE PlayerID = '%s'", curX, curY, clientID);

        retcode = SQLExecDirectA(hstmt, (SQLCHAR*)query, SQL_NTS);

        if (hstmt)
        {
            SQLFreeHandle(SQL_HANDLE_STMT, hstmt);
        }
        std::cout << "[DB 위치 저장] " << clientID << " (" << curX << ", " << curY << ")\n";
    }
}
...
void process_packet(int c_id, char* packet)
{
    ...
    case C2S_LOGIN: {
        ...
        float dbX = 0.0f, dbY = 0.0f;

        if (HandleLoginPacket(p->username, dbX, dbY))
        {
            std::cout << p->username << " 로그인 성공! 과거 위치: (" << dbX << ", " << dbY << ")\n";

            {
                lock_guard<mutex> ll{ clients[c_id]->_s_lock };
                clients[c_id]->_x = static_cast<int>(dbX);
                clients[c_id]->_y = static_cast<int>(dbY);
        ...
        {
            std::cout << p->username << " 로그인 실패! (DB에 등록되지 않은 ID)\n";
...
void disconnect(int c_id)
{
    ...
    {
        SavePlayerPosition(clients[c_id]->_name, clients[c_id]->_x, clients[c_id]->_y);
    }
...
bool ConnectDB()
{
    ...
    SQLCHAR dsn[] = "GameServer_DSN";
    SQLRETURN ret = SQLConnectA(hDbc, dsn, SQL_NTS, NULL, 0, NULL, 0);
