...
void SESSION::do_move(DIRECTION dir)
{
	...
	for (auto& c : clients)
	{
		std::shared_ptr<SESSION> other = c.second.load();
		if (nullptr == other) continue;
		if (other->m_state != CS_PLAYING) continue;
		if (other->m_id == m_id) continue;

		// 전체 유저 중 내 시야 반경에 있는 사람만 추려냄
		if (is_visible(other->m_x, other->m_y))
		{
			new_v.insert(other->m_id);
		}
	}

	// 예전 리스트에 있던 id가 새로운 리스트에 없다면..
	for (int other_id : old_v)
	{
		if (new_v.count(other_id) == 0)	// 시야에서 사라짐
		{
			// 내 시야 리스트에서 지우고 클라이언트에 remove 패킷을 보낸다
			{
				std::lock_guard<std::mutex> lock(m_visible_mutex);
			...
			// 상대방 시야에 내가 진짜 있었다면, 상대방 클라에 나를 지우기
			if (is_in_other_view)
			{
	...
	for (int id : new_v)
	{
		...
		bool is_in_other_view = false;
		{
			...
			{ 
				pl->m_visible_players.insert(m_id);
				is_in_other_view = false;
			}
		}
...
bool SESSION::process_packet(unsigned char* p)
{
	...
	case C2S_LOGIN: {
		...
		for (auto& c : clients)
		{
			...
			send_add_player(pl->m_id);	// 시야 안에 있는 player만 add player
			pl->send_add_player(m_id);
		}
	}
				  break;
