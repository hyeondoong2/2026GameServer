...
void ProcessPacket(char* ptr)
{
	...
	{
		S2C_AvatarInfo_Pakcet* packet = reinterpret_cast<S2C_AvatarInfo_Pakcet*>(ptr);
		g_myid = packet->player_id;
		avatar.m_x = packet->x;
		avatar.m_y = packet->y;
		avatar.show();
	}
	...
	case S2C_ADD_PLAYER:
	{
		S2C_AddPlayer_Packet* my_packet = reinterpret_cast<S2C_AddPlayer_Packet*>(ptr);
		int id = my_packet->player_id;
		if (id >= MAX_USER) {
			std::cout << "Too many users! id : " << id << std::endl;
			exit(-1);
		}
		players[id].move(my_packet->x, my_packet->y);
		players[id].set_name(my_packet->user_name);
		players[id].show();
		break;
	}
	case S2C_MOVE_PLAYER:
	{
		S2C_MovePlayer_Packet* my_packet = reinterpret_cast<S2C_MovePlayer_Packet*>(ptr);
		int other_id = my_packet->player_id;
		if (other_id == g_myid) {
	...
	{
		S2C_ReomvePlayer_Packet* my_packet = reinterpret_cast<S2C_ReomvePlayer_Packet*>(ptr);
		int other_id = my_packet->player_id;
		if (other_id == g_myid)
			avatar.hide();
		else if (other_id < MAX_USER)
